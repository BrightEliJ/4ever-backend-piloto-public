...
import tim.field.application.TarefasWFM.model.BacklogActivitiesHistoric;

public interface BacklogActivitiesHistoricRepository extends JpaRepository<BacklogActivitiesHistoric, Long> {
    ...
    @Query(value = "SELECT b.\"apptNumber\", b.\"XA_PI_EVENT\", b.\"XA_PI_CREATE_DATE\", " +
            ...
            "FROM backlog_activities_historic b " +
        ...
        countQuery = "SELECT COUNT(*) FROM backlog_activities_historic b " +
            "WHERE (:search IS NULL OR " +
            "b.\"apptNumber\" ILIKE CONCAT('%', :search, '%') OR " +
            "b.status ILIKE CONCAT('%', :search, '%') OR " +
            "b.regional ILIKE CONCAT('%', :search, '%') OR " +
    ...
    @Query(value = "SELECT COUNT(*) FROM backlog_activities_historic b " +
            "WHERE (:search IS NULL OR " +
            "b.\"apptNumber\" ILIKE CONCAT('%', :search, '%') OR " +
            "b.status ILIKE CONCAT('%', :search, '%') OR " +
            "b.regional ILIKE CONCAT('%', :search, '%') OR " +
